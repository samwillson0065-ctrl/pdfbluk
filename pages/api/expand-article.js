...
export default async function handler(req,res){
  ...
  const completion=await openai.chat.completions.create({
    model:"gpt-4o-mini",
    messages:[{role:"system",content:"You are a skilled blog writer."},{role:"user",content:prompt}],
    max_tokens:1600,temperature:0.9});
  const content=completion.choices[0].message.content||"";
  res.status(200).json({title:fullTitle,filename:sanitizeFileName(outline.filename.replace(".pdf",""))+".pdf",content});
}